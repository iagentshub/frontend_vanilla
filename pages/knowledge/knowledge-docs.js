...
var KnowledgeDocs = (function () {
    ...
    function _render() {
        ...
        grid.innerHTML = slice.map(function (item) {
            ...
            if (!item._shared) {
                // Es mío (propietario)
                ownerBadge2 = '<span class="label-chip" style="--lc:#059669">' +
                    esc(t('agents.origin.owner') || 'Propietario') + '</span>';
            } else if (window._activeGroupKnId) {
                // Compartido en grupo: mostrar quién lo compartió
                var ol2 = item.owner_id ? '@' + item.owner_id : (t('teams.sharing.shared_badge') || 'Compartido');
                ownerBadge2 = '<span class="res-badge res-badge--shared">' + esc(ol2) + '</span>';
            }
